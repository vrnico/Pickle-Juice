...
- [x] 3.1 Home screen with idle state (Consume Start / Create Start buttons)
- [x] 3.2 Home screen active-session view with running elapsed time and Stop button
- [x] 3.3 Confirmation prompt when Start is tapped while a session is active
- [x] 3.4 Dashboard view with today's ratio bar, 7-day ratio bar, and totals in minutes
- [x] 3.5 Dashboard empty state with "start your first session" call to action
- [x] 3.6 History view grouped by day, reverse-chronological, showing category/duration/start
- [x] 3.7 Edit-session dialog with category + start/end inputs and validation ("end must be after start")
- [x] 3.8 Delete-session flow with confirmation
- [x] 3.9 Settings screen with CSV Export button and empty-state error for zero sessions
- [x] 3.10 Bottom nav / tab switcher (Home / Dashboard / History / Settings)
- [x] 3.11 Install prompt hint on supported browsers

## 4. PWA shell (W2 — pwa-ui worktree)

- [x] 4.1 Author `public/manifest.webmanifest` with name, short_name, icons (192/512), start_url, display=standalone, theme/background colors
- [x] 4.2 Generate icon set (at least 192px and 512px maskable)
- [x] 4.3 Register service worker for offline shell caching (Next.js App Router compatible approach)
